...
const Card: React.FC<CardProps> = ({
  ...
  return (
    ...
    >
      {/* Content */}
      <div>
        <h3 className="text-lg sm:text-xl md:text-2xl font-semibold leading-snug mb-2 sm:mb-3">
          {title}
        </h3>
      ...
      {/* Button */}
      <button className="mt-6 sm:mt-8 w-full sm:w-fit bg-white text-gray-800 px-4 py-2 rounded-md text-sm sm:text-md font-medium hover:bg-gray-100 transition">
        {buttonText}
      </button>
    </motion.div>
  );
...
const WhatWeOffer = () => {
  return (
    <section className="py-12 sm:py-14 md:py-16 px-4 sm:px-6 md:px-12 lg:px-20 xl:px-28">
      {/* HEADER */}
      <motion.div
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
      ...
      <motion.div
        ...
        whileInView="show"
        viewport={{ once: true }}
        className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-5 md:gap-6 max-w-6xl mx-auto"
      >
        <Card
          title="Upgrade existing STPs to our NaturalSTP system"
          description="Convert your underperforming or high-cost conventional STP into an efficient NaturalSTP system."
      ...
      <motion.div
        ...
        transition={{ duration: 0.6, delay: 0.2 }}
        viewport={{ once: true }}
        className="max-w-6xl mx-auto mt-10 sm:mt-14 md:mt-16 rounded-xl py-4 sm:py-6 flex flex-col md:flex-row items-center justify-between gap-6"
      >
        {/* TEXT */}
        <p className="text-gray-600 text-xs sm:text-sm md:text-base max-w-sm text-center md:text-left">
          Companies that switch to NaturalSTP systems have seen huge benefits
          and improvements in ROI and Hasslefree experience.
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 items-center">
          ...
          <motion.div
            ...
            className="flex items-center gap-3"
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-gray-800 flex items-center justify-center shadow-md">
